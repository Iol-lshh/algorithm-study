...
public class Dijkstra {
    ...
    static int[] dijkstra(int[][] graph, int start) {
        // + 최소 경로 리스트를 구하고 싶다면?
        List<List<Integer>> visitedProcess = new ArrayList();
        for(int i = 0; i< graph.length; i++){
            visitedProcess.add(new ArrayList<>());
            visitedProcess.get(i).add(start);
            visitedProcess.get(i).add(i);
        }
        ...
        while (node != -1) {
            ...
            for (int i = 0; i < neighbors.length; i++) {
                if(costs[i] > costs[node] + graph[node][i]){
                    ...
                    // 경로 추가
                    visitedProcess.set(i, new ArrayList<>(visitedProcess.get(node)));
                    visitedProcess.get(i).add(i);
                }
            }
        ...
        System.out.println(visitedProcess);
        return costs;
    }
